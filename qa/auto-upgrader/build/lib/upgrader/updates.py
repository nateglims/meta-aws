...
def _check_for_updates(recipe: str) -> Optional[dict]:
    ...
    if m:
        ...
        return {"name": recipe, "previous_version": m.group(1), "next_version": m.group(2)}
    else:
    ...
    for recipe in _find_recipes(layer_path):
        ...
        if upgrade:
            if upgrade.get("next_version") == "^{}":
                logger.error(f"error getting ref for {recipe}")
                continue
            new_branch = f"{date}T" f"_{target_branch}_{upgrade.get('name')}"
            commit_msg = (
                f"{upgrade.get('name')}: upgrade {upgrade.get('previous_version')} "
                f"-> {upgrade.get('next_version')}"
            )
            run(f"git -C {layer_path} checkout -b {new_branch} {target_branch}")
            run(f"devtool finish --force --force-path-refresh {upgrade.get('name')} {layer_path}")
